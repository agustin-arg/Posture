...
class PostureActuators:
    ...
    def _update_servo(self, is_bad_posture, enabled):
        ...
        if (
            self._servo_candidate_state is None
            or self._servo_candidate_state != observed
        ):
            self._servo_candidate_state = observed
            self._servo_candidate_since = now

        if (
            utime.ticks_diff(now, self._servo_candidate_since)
            >= config.SERVO_DEBOUNCE_MS
        ):
            if self._servo_last_applied_state != self._servo_candidate_state:
                target_angle = (
                    config.SERVO_ALERT_ANGLE
                    if self._servo_candidate_state
                    else config.SERVO_IDLE_ANGLE
                )
                try:
        ...
        if (
            utime.ticks_diff(now, self._servo_hold_until) >= 0
            and self._servo_hold_until != 0
        ):
            try:
                self.servo.duty(0)
                self._servo_hold_until = 0
