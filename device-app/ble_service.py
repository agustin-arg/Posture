...
class PostureBLE:
    ...
    def _setup_services(self):
        ...
        status_char = (
            config.POSTURE_STATUS_CHAR_UUID,
            ubluetooth.FLAG_READ | ubluetooth.FLAG_NOTIFY,
        )
        threshold_char = (
            config.THRESHOLD_ANGLE_CHAR_UUID,
            ubluetooth.FLAG_READ | ubluetooth.FLAG_WRITE,
        )
        calibrate_char = (
            config.CALIBRATE_CHAR_UUID,
            ubluetooth.FLAG_WRITE,
        )
        buzzer_char = (
            config.BUZZER_CONTROL_CHAR_UUID,
            ubluetooth.FLAG_READ | ubluetooth.FLAG_WRITE,
        )
        vibrator_char = (
            config.VIBRATOR_CONTROL_CHAR_UUID,
            ubluetooth.FLAG_READ | ubluetooth.FLAG_WRITE,
        )
        leds_char = (
            config.LEDS_CONTROL_CHAR_UUID,
            ubluetooth.FLAG_READ | ubluetooth.FLAG_WRITE,
        )
        notify_char = (
            config.NOTIFY_CONTROL_CHAR_UUID,
            ubluetooth.FLAG_READ | ubluetooth.FLAG_WRITE,
        )
        system_char = (
            config.SYSTEM_CONTROL_CHAR_UUID,
            ubluetooth.FLAG_READ | ubluetooth.FLAG_WRITE,
        )
        battery_char = (
        ...
        (
            ...
            self.calibrate_handle,
            self.buzzer_handle,
            self.vibrator_handle,
            self.leds_handle,
            self.notify_handle,
            self.system_handle,
            self.battery_handle,
    ...
    def update_system_state_on_characteristic(self, state):
        ...
        try:
            self.ble.gatts_write(
                self.system_handle, struct.pack("<B", 1 if state else 0)
            )
        except:
            pass
